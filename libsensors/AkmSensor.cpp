...
AkmSensor::AkmSensor()
: SensorBase(AKM_DEVICE_NAME, "sensors"),
      mEnabled(0),
      mPendingMask(0),
      mInputReader(32)
{
...
//    mPendingEvents[Light  ].Light.status = SENSOR_STATUS_ACCURACY_HIGH;
    ...
    if (!ioctl(dev_fd, ECS_IOCTL_APP_GET_TFLAG, &flags)) {
        if (flags)  {
            ...
            if (!ioctl(data_fd, EVIOCGABS(EVENT_TYPE_TEMPERATURE), &absinfo)) {
                mPendingEvents[Temp].temperature = absinfo.value;
            } 
        }
    }
...
/*
    ...
    {
	ALOGE("Light SENSORS_GET_LUX_FIR success");
        int lightval;
        if (!ioctl(data_fd, EVENT_TYPE_LIGHT, &lightval)) {
            
             ALOGE("light return: %d",lightval);
             mPendingEvents[Light].light = lightval;
        }
    }
    mPendingEvents[Light].light = 70;
*/
    ALOGV("AkmSensor %d " ,mEnabled);
...
//            case Light:         cmd = SENSORS_GET_LUX_FIR; newState = 1; ioctl(dev_fd, cmd, &newState);       break;
            case Temp:          cmd = ECS_IOCTL_APP_SET_TFLAG;  break;
        }
        short flags = newState;
        err = ioctl(dev_fd, cmd, &flags);
        err = err<0 ? -errno : 0;
        ALOGE_IF(err, "ECS_IOCTL_APP_SET_ %d failed (%s)",cmd, strerror(-err));
        ...
        ALOGV("AkmSensor::enable %d %d " ,handle, mEnabled);
...
int AkmSensor::update_delay()
{
    if (mEnabled) {
	    ...
	    ALOGV("ECS_IOCTL_APP_SET_DELAY failed");
            return -errno;
        }
    }
    return 0;
...
{
    ALOGV("AkmSensor::readEvents %d", count);
    if (count < 1)
        return -EINVAL;
...
//            mPendingMask |= 1<<Light;
            ...
            ALOGE("AkmSensor: light event (type=%d, code=%d, val: %d)", type, event->code, event->value);
...
{
    ALOGV("AkmSensor::processEvent %d %d " ,code, value);
    ...
    switch (code) {
        ...
        case EVENT_TYPE_TEMPERATURE:
            mPendingMask |= 1<<Temp;
            mPendingEvents[Temp].temperature = value;
            break;
    }
}
